...
## Known listing blocker (not changed here)

`targetSdk` / `compileSdk` are **34**. As of 31 August 2026, new Play phone/tablet listings and updates must target **API 36**. Bumping SDK is a behavior-change PR (edge-to-edge, 16 KB pages, FGS / IME). Track it as a follow-up; do not treat a signed API-34 AAB as Console-submittable after that date.

R8/minify stays **off** for release so Vault/IME/Room keep current behavior. Enabling shrinking is a separate regression pass.
